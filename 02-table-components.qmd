...
The `rowname_col` argument is particularly powerful because it can accept multiple column names, creating a hierarchical stub layout. When you provide a vector of column names, values from the leftmost column form the highest level of the hierarchy, proceeding to individual row identifiers in the rightmost column. This multi-column stub feature is useful for displaying nested categories or hierarchical data structures.

The `omit_na_group` argument controls how rows with `NA` values in the `groupname_col` are handled. By default (`FALSE`), such rows are assigned to a group labeled `"NA"`. Setting this to `TRUE` causes those rows to appear as ungrouped rows in the table body, which is useful when you want to include header or separator rows that shouldn't belong to any row group.
...
pizzaplace |>
  ...
  tab_source_note(
    source_note = md("Data from the `pizzaplace` dataset in **gt**")
  ) |>
...
As you progress through subsequent chapters, you'll see how these structural foundations support more advanced capabilities. *Chapters 3 and 4* cover formatting functions for numeric and non-numeric data, building upon the column organization you establish here. *Chapter 5* introduces substitution and text transformation, completing the three-stage rendering pipeline. *Chapters 6 and 7* address column modifications and summary rows, extending the structural concepts introduced in this chapter. The styling techniques in *Chapter 8* leverage the component structure to apply visual enhancements precisely where needed, while *Chapter 9* shows how to add footnotes that reference structural elements. *Chapter 10* demonstrates nanoplots for embedding visualizations within cells. The advanced topics in *Chapters 11 and 12* cover table groups and output formats, and *Chapters 13 and 14* explore how to extend **gt** through external packages and your own extensions. All of these capabilities depend on the solid structural foundation established by the basic components covered here.
