...
dplyr::tibble(
  Notation = c("`^`", "`_`", "`()`", "`-`"),
  Meaning = c("Superscript (exponent)", "Subscript", "Grouping", "Minus in exponent"),
  `Example Input` = c("`m^2`", "`x_0`", "`J / (kg K)`", "`m^-1`"),
  `Renders As` = `Example Input`
) |>
  gt() |>
  fmt_markdown(columns = c(Notation, `Example Input`)) |>
  fmt_units(columns = `Renders As`)
```
