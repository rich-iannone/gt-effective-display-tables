...
pizzaplace |>
  ...
  dplyr::summarize(
    ...
    price_range = dplyr::if_else(
      min(price) == max(price),
      sprintf("$%.2f", min(price)),
      paste0("$", sprintf("%.2f", min(price)), "\u2013$", sprintf("%.2f", max(price)))
    ),
    .groups = "drop"
  ) |>
  dplyr::mutate(
...
exibble |>
  gt() |>
  tab_header(
    title = md("The `exibble` dataset"),
    subtitle = "8 rows and 9 columns."
  ) |>
  tab_style(style = cell_fill(color = "#F8F8F8"), locations = cells_body(columns = num)) |>
  tab_style(style = cell_fill(color = "#F2F2F2"), locations = cells_body(columns = char)) |>
  tab_style(style = cell_fill(color = "#ECECEC"), locations = cells_body(columns = fctr)) |>
  tab_style(style = cell_fill(color = "#E6E6E6"), locations = cells_body(columns = date)) |>
  tab_style(style = cell_fill(color = "#E0E0E0"), locations = cells_body(columns = time)) |>
  tab_style(style = cell_fill(color = "#DADADA"), locations = cells_body(columns = datetime)) |>
  tab_style(style = cell_fill(color = "#D4D4D4"), locations = cells_body(columns = currency)) |>
  tab_style(style = cell_fill(color = "#CECECE"), locations = cells_body(columns = row)) |>
  tab_style_body(
    style = cell_text(color = "red3", weight = "bold"),
    fn = function(x) is.na(x)
  ) |>
  tab_style(style = cell_fill(color = "#C8C8C8"), locations = cells_body(columns = group))
```
...
# Helper to create inline SVG logo for manufacturer row group labels
mfr_logo <- function(mfr_name, svg_file, height = 18) {
  svg_text <- paste(
    readLines(paste0("assets/logos/", svg_file), warn = FALSE),
    collapse = " "
  )
  svg_text <- gsub('<\\?xml[^?]*\\?>', '', svg_text)
  svg_text <- gsub('<!--.*?-->', '', svg_text)
  svg_text <- gsub(' width="[^"]*"', '', svg_text)
  svg_text <- gsub(' height="[^"]*"', '', svg_text)
  svg_text <- sub(
    '<svg ',
    paste0('<svg height="', height, '" width="', height, '" '),
    svg_text
  )
  html(paste0(
...
gtcars |>
  ...
  dplyr::select(ctry_origin, mfr, model, hp, trq, mpg_c, mpg_h, msrp) |>
  dplyr::arrange(factor(ctry_origin, levels = c("Germany", "Italy")), mfr, model) |>
  gt(rowname_col = "model") |>
  tab_row_group(
    label = mfr_logo("Audi", "audi.svg"),
    rows = mfr == "Audi", id = "Audi"
  ) |>
  tab_row_group(
    label = mfr_logo("BMW", "bmw.svg"),
    rows = mfr == "BMW", id = "BMW"
  ) |>
  tab_row_group(
    label = mfr_logo("Mercedes-Benz", "merc.svg"),
    rows = mfr == "Mercedes-Benz", id = "Mercedes-Benz"
  ) |>
  tab_row_group(
    label = mfr_logo("Porsche", "porsche.svg"),
    rows = mfr == "Porsche", id = "Porsche"
  ) |>
  tab_row_group(
    label = mfr_logo("Ferrari", "ferrari.svg"),
    rows = mfr == "Ferrari", id = "Ferrari"
  ) |>
  tab_row_group(
    label = mfr_logo("Lamborghini", "lamborghini.svg"),
    rows = mfr == "Lamborghini", id = "Lamborghini"
  ) |>
  tab_row_group(
    label = mfr_logo("Maserati", "maserati.svg"),
    rows = mfr == "Maserati", id = "Maserati"
  ) |>
  row_group_order(
    groups = c(
  ...
  cols_hide(columns = c(ctry_origin, mfr)) |>
  cols_label(
    hp = "HP",
    trq = "Torque",
    mpg_c = "City",
  ...
  cols_width(
    stub() ~ px(160),
    c(hp, trq, mpg_c, mpg_h) ~ px(75),
    msrp ~ px(100)
  ) |>
  tab_style(
    style = cell_fill(color = "#EBF5FB"),
    locations = cells_body(rows = ctry_origin == "Germany")
  ) |>
  tab_style(
    style = cell_fill(color = "#FEF9E7"),
    locations = cells_body(rows = ctry_origin == "Italy")
  ) |>
  tab_header(
    title = "German and Italian Grand Tourers",
    subtitle = "Precision engineering meets Mediterranean passion."
  ) |>
  opt_all_caps() |>
...
gtcars |>
  ...
  tab_header(
    title = md("Most Expensive Cars in `gtcars` &#x1F4B0;"),
    subtitle = "Top 10 by manufacturer's suggested retail price."
  )
```
...
gtcars |>
  ...
  cols_label(
    count = "Models",
    avg_torque = md("Avg Torque<br>(lb-ft)"),
    avg_price = "Avg Price"
  ) |>
  cols_width(
    stub() ~ px(200),
  ...
  tab_header(
    ...
    subtitle = md("Grouping the 47 cars in `gtcars` by horsepower output.")
  )
```
...
countrypops |>
  ...
  dplyr::mutate(flag = country_code_3) |>
  dplyr::select(country_code_3, flag, population) |>
  gt(rowname_col = "country_code_3") |>
  fmt_number(columns = population, n_sigfig = 3, suffixing = TRUE) |>
...
towny |>
  dplyr::select(name, population_2001, density_2001, population_2021, density_2021) |>
  dplyr::slice_max(population_2021, n = 10) |>
  gt(rowname_col = "name") |>
  fmt_integer(columns = c(population_2001, population_2021)) |>
  fmt_number(columns = c(density_2001, density_2021), decimals = 1) |>
  cols_label(
    population_2001 = "Population",
    density_2001 = "Density",
    population_2021 = "Population",
    density_2021 = "Density"
  ) |>
  tab_spanner(label = "2001", columns = c(population_2001, density_2001)) |>
  tab_spanner(label = "2021", columns = c(population_2021, density_2021)) |>
  cols_width(everything() ~ px(110)) |>
  tab_header(
    title = "Ontario's Largest Municipalities",
    subtitle = "Population and density for the top 10, 2001 vs. 2021."
  ) |>
  tab_source_note("Density is measured in persons per km².")
```
...
peeps |>
  ...
  cols_add(flag = country) |>
  fmt_flag(columns = flag) |>
  cols_move(columns = flag, after = country) |>
  cols_label(
  ...
  tab_header(title = "A Random Selection of Peeps")
```
...
sza |>
  ...
  dplyr::mutate(
    latitude = paste0(latitude, "\u00BAN"),
    tst = paste0(substr(tst, 1, 2), ":", substr(tst, 3, 4))
  ) |>
  tidyr::pivot_wider(names_from = month, values_from = sza) |>
  gt(rowname_col = "tst", groupname_col = "latitude") |>
  fmt_number(decimals = 0, pattern = "{x}\u00B0") |>
  sub_missing(missing_text = "—") |>
  tab_header(
...
constants |>
  dplyr::filter(name %in% c(
    ...
    "elementary charge", "Boltzmann constant",
    "Avogadro constant", "fine-structure constant",
    "electron mass", "proton mass",
    "Bohr radius", "Newtonian constant of gravitation"
  )) |>
  ...
  tab_header(
    title = "Numbers That Define the Universe",
    subtitle = "Ten fundamental physical constants."
  )
```
...
crash_week <-
  sp500 |>
  dplyr::filter(date >= "1987-10-14", date <= "1987-10-23") |>
  dplyr::arrange(date) |>
  dplyr::mutate(
    day = format(date, "%A"),
    label = as.character(date)
  ) |>
  dplyr::select(label, day, open, close, volume)

weekend_row <- dplyr::tibble(
  label = "The Weekend", day = "",
  open = NA_real_,
  close = NA_real_, volume = NA_real_
)

dplyr::bind_rows(
  crash_week |> dplyr::filter(label <= "1987-10-16"),
  weekend_row,
  crash_week |> dplyr::filter(label >= "1987-10-19")
) |>
  gt(rowname_col = "label") |>
  fmt_currency(columns = c(open, close), decimals = 2) |>
  ...
  tab_style(
    ...
    locations = cells_body(rows = label == "The Weekend")
  ) |>
  tab_style(
    style = cell_text(color = "#D73027", weight = "bold"),
    locations = cells_body(rows = label == "1987-10-19")
  ) |>
  tab_style(
    style = list(
      cell_fill(color = "#f0f0f0"),
    ...
    locations = cells_stub(rows = label == "The Weekend")
  ) |>
  tab_style(
  ...
  tab_header(
    title = "Black Monday and the Week That Shook Wall Street",
    subtitle = "S&P 500 daily prices, October 14\u201323, 1987."
  )
```
...
sp500 |>
  ...
  dplyr::summarize(
    ...
    year_open = dplyr::first(open),
    year_close = dplyr::last(close),
    year_high = max(high),
    year_low = min(low),
  ...
  cols_label(
    year_open = "Open",
    year_close = "Close",
    return_pct = md("Annual<br>Return"),
    year_high = "Year High",
    year_low = "Year Low"
  ) |>
  cols_width(
...
dplyr::tibble(
  ...
  automated = c(TRUE, FALSE, FALSE, FALSE, TRUE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, TRUE)
) |>
  gt() |>
  cols_label(
    line = "Line",
  ...
  fmt_tf(columns = automated, tf_style = "check-mark") |>
  data_color(
    columns = color,
    fn = function(x) x
  ) |>
...
gibraltar |>
  dplyr::filter(date == "2023-05-01", time >= "06:00", time <= "14:00") |>
  dplyr::select(time, temp, humidity, wind_speed, wind_dir, condition) |>
  gt() |>
  fmt_number(columns = temp, decimals = 1) |>
  fmt_percent(columns = humidity, decimals = 0) |>
  fmt_number(columns = wind_speed, decimals = 1) |>
  cols_label(
    time = "Time",
    temp = "Temp (°C)",
    humidity = "Humidity",
  ...
  tab_header(
    title = "Gibraltar Morning Weather",
    subtitle = "May 1, 2023: fog clearing to fair skies."
  ) |>
  opt_vertical_padding(scale = 0.5)
```
...
films |>
  ...
  dplyr::select(title, director, countries_of_origin) |>
  gt() |>
  ...
  tab_header(
    title = "Cannes Film Festival 2019",
    subtitle = "Official Competition"
  )
```
...
films |>
  dplyr::filter(!grepl(",", countries_of_origin)) |>
  dplyr::count(countries_of_origin, sort = TRUE) |>
  dplyr::slice_head(n = 12) |>
  gt(rowname_col = "countries_of_origin") |>
  fmt_country(columns = countries_of_origin) |>
  cols_label(n = "Films in Competition") |>
  tab_header(
    title = "Countries by Cannes Competition Entries",
    subtitle = "Single-country productions across festival history."
  )
```

The table above reveals which national cinemas have received the most recognition at Cannes. The United States leads with 201 films, followed by France with 150 (still a strong showing given Cannes is a French festival on the French Riviera). The United Kingdom and Italy follow, with all four countries having robust film industries and strong traditions of auteur filmmaking. Japan's presence reflects the festival's long appreciation for Japanese cinema, from early masters like Kinugasa, Mizoguchi, and Kobayashi through to Imamura, Kitano, and more recently Kore-eda and Hamaguchi. The geographic diversity of competition entries has increased over time, with films from Korea, Iran, Thailand, and other countries appearing regularly in recent decades.
