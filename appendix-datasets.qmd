...
crash_week <-
  ...
  dplyr::arrange(date) |>
  dplyr::mutate(day = format(date, "%A")) |>
  dplyr::select(date, day, open, high, low, close, volume)

weekend_row <- dplyr::tibble(
  date = as.Date("1987-10-17"), day = "Weekend",
  open = NA_real_, high = NA_real_, low = NA_real_,
  close = NA_real_, volume = NA_real_
)

dplyr::bind_rows(
  crash_week |> dplyr::filter(date <= "1987-10-16"),
  weekend_row,
  crash_week |> dplyr::filter(date >= "1987-10-19")
) |>
  gt(rowname_col = "date") |>
  ...
  tab_style(
    ...
    locations = cells_body(rows = day == "Weekend")
  ) |>
  tab_style(
    style = cell_text(color = "#D73027", weight = "bold"),
    locations = cells_body(rows = date == as.Date("1987-10-19"))
  ) |>
  tab_style(
    style = list(
      cell_fill(color = "#f0f0f0"),
    ...
    locations = cells_stub(rows = day == "Weekend")
  ) |>
  tab_header(
