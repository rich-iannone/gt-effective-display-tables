...
For more subtle control, HTML provides two additional mechanisms. The `<wbr>` tag (word break opportunity) suggests a point where the browser *may* break a long word if the line needs wrapping, but adds no visible character. The soft hyphen `&shy;` works similarly but inserts a visible hyphen when the break occurs:

```{r}
#| echo: false

dplyr::tibble(
  technique = c("No break hints", "With <wbr> tags", "With &shy; soft hyphens"),
  content = c(
    "<span style='font-family:monospace;'>supercalifragilisticexpialidocious</span>",
    "<span style='font-family:monospace;'>super<wbr>cali<wbr>fragilistic<wbr>expialidocious</span>",
    "<span style='font-family:monospace;'>super&shy;cali&shy;fragilistic&shy;expialidocious</span>"
  )
) |>
  gt() |>
  fmt_passthrough(columns = content, escape = FALSE) |>
  cols_width(everything() ~ px(180)) |>
  cols_label(technique = "Technique", content = "Content") |>
  tab_header(
    title = "Word break strategies",
    subtitle = "Narrow column forces wrapping"
  )
```

The `<wbr>` tag is especially useful for long identifiers, URLs, or file paths in table cells. It lets the browser break at sensible points without adding visible punctuation. Soft hyphens (`&shy;`) suit natural language words that follow standard hyphenation rules. Both tags are invisible when no break occurs, which is ideal since the break points are only suggestions.

### CSS text layout properties

CSS provides several properties that change how text flows within a cell. These properties can be applied through `tab_style()` with `css()` or through `opt_css()` when you need to target elements that `tab_style()` does not directly cover.

The `text-wrap: balance` property distributes text evenly across multiple lines so that no line is dramatically longer than the others. This is epecially effective for table titles and subtitles where unbalanced wrapping can look awkward:

```{r}
exibble |>
  dplyr::select(char, num) |>
  dplyr::slice_head(n = 3) |>
  gt() |>
  tab_header(
    title = html(
      "<span style='text-wrap: balance;'>A Reasonably Long Table Title
      That Would Otherwise Wrap Unevenly Across Multiple Lines</span>"
    )
  )
```

Without `text-wrap: balance`, the browser fills each line to the maximum width before wrapping, often leaving a single short word orphaned on the last line. Balanced wrapping distributes the text to produce lines of roughly equal length, creating a more polished appearance.

Other useful CSS text properties include `text-wrap: pretty`, which specifically avoids orphaned single words on the last line. The `word-break: break-all` property allows breaking within any word (useful for columns containing long codes or identifiers). And `overflow-wrap: break-word` breaks long words only when they would overflow their container, which is less aggressive than `break-all`:

```{r}
dplyr::tibble(
  property = c("Default", "word-break: break-all", "overflow-wrap: break-word"),
  content = c(
    "A verylongstringwithoutspaces can overflow",
    "<span style='word-break: break-all;'>A verylongstringwithoutspaces can overflow</span>",
    "<span style='overflow-wrap: break-word;'>A verylongstringwithoutspaces can overflow</span>"
  )
) |>
  gt() |>
  fmt_passthrough(columns = content, escape = FALSE) |>
  cols_width(everything() ~ px(180)) |>
  cols_label(property = "CSS Property", content = "Result") |>
  tab_header(title = "CSS word-breaking strategies")
```

### Building complex headers and footers with divs

The `tab_header()` and `tab_source_note()` functions accept `html()` content, which means you can use `<div>` elements with CSS to create sophisticated layouts. This is particularly useful when a title needs to contain multiple pieces of information arranged side by side, or when a footer needs a left-aligned source and a right-aligned timestamp.

A common pattern uses CSS flexbox to create a two-column header:
  ...
  tab_header(
    title = html(
      "<div style='display: flex; justify-content: space-between; align-items: baseline;'>
        <div style='font-size: 1.1em;'>Quarterly Summary</div>
        <div style='font-size: 0.75em; color: #888;'>FY 2025 &middot; Q3</div>
      </div>"
    ),
    subtitle = html(
      "<div style='display: flex; justify-content: space-between;'>
        <div>Eastern Region</div>
        <div style='font-size: 0.85em; color: #666;'>Updated: 2025-09-15</div>
      </div>"
    )
  )
```

The outer `<div>` with `display: flex` and `justify-content: space-between` creates a flex container that pushes its children to opposite ends. The left `<div>` gets the main title text, and the right `<div>` gets supplementary information. This pattern is impossible with plain text and difficult to achieve with Markdown alone.

The same technique works for source notes that need structured layouts:
  ...
  tab_source_note(
    source_note = html(
      "<div style='display: flex; justify-content: space-between; width: 100%;'>
        <div>Source: Company financial records</div>
        <div style='color: #999;'>Confidential</div>
      </div>"
    )
  ) |>
  tab_source_note(
    source_note = html(
      "<div style='display: flex; justify-content: space-between; width: 100%;'>
        <div style='font-style: italic;'>Figures in USD unless noted</div>
        <div style='color: #999;'>Generated: 2025-09-15</div>
      </div>"
    )
  )
```

For more complex arrangements, you can nest divs further. A three-column footer, a header with an icon or badge, or a stacked layout with a main title above a row of metadata fields are all achievable. The key principle is that `html()` gives you the full power of HTML and CSS within any text-accepting **gt** function. The inner elements inherit some styling from **gt**'s table structure, but your inline styles take precedence for the properties you specify.

One caution: these HTML layouts are specific to HTML output. If you need your table to also render in LaTeX, Word, or RTF, the `<div>` structures will not translate. For multi-format tables, keep headers and footers simple or use conditional logic to provide different content per format.
