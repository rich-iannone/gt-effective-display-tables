...
## Combining tables and plots with patchwork

Tables and plots often complement each other: a plot reveals trends and patterns at a glance, while a table provides precise values for verification and reference. Traditionally, combining **gt** tables with **ggplot2** plots required workarounds: saving each to a file and assembling them in external software, or arranging them in a document outside of R. The `as_gtable()` function changes this by converting **gt** tables into grid graphics objects, enabling composition with plots using the **patchwork** package.

The grid graphics system is the foundation for **ggplot2**'s rendering. By converting a **gt** table to a grid table object (a "gtable"), the table becomes composable with any grid-based graphic. This opens up possibilities for creating publication-ready figures that integrate detailed tables with their corresponding visualizations, all generated and combined within R.

### Basic composition

The simplest way to combine a table and plot uses **patchwork**'s intuitive `+` operator. When you add a **gt** table to a **ggplot** object, **patchwork** automatically converts the table and arranges both elements side by side:
...
pizza_plot <-
  ...
  ggplot(aes(x = date, y = sold, color = type)) +
  geom_line() +
  scale_x_date(date_labels = "%b", date_breaks = "2 months") +
  labs(x = NULL, y = "Pizzas Sold", color = "Type") +
  theme_minimal()

# Combine them
pizza_plot + pizza_table
```

The `+` operator places elements horizontally. For vertical arrangements, use `/`:

```{r}
pizza_plot / pizza_table
```

### Controlling table size and alignment

Tables have fixed dimensions determined by their content, while plots expand to fill available space. This mismatch can create awkward layouts where tables overflow their allocated area or leave excessive whitespace. The `wrap_table()` function from **patchwork** provides fine-grained control over how tables occupy space in the composition.

The `space` argument determines whether the table's dimensions constrain the layout:

```{r}
# Let the table width be fixed, height flexible
pizza_plot + wrap_table(pizza_table, space = "free_y")
```

Setting `space = "free_y"` fixes the table's width to its natural size but allows the height to adjust to the plot's panel height. This prevents horizontal overflow while maintaining vertical alignment. Other options include `"free"` (both dimensions free), `"free_x"` (height fixed), and `"fixed"` (both dimensions fixed to the table's natural size).

The `panel` argument controls alignment between the table's structure and the plot's panel area. By default (`panel = "body"`), the table body aligns with the plot panel, placing headers and row names outside the aligned region. This creates natural alignment when the table supplements the plot:

```{r}
# Align table body with plot panel (default)
pizza_plot + wrap_table(pizza_table, space = "free_y", panel = "body")
```

### Tables as axis supplements

A powerful application is using tables to replace or supplement axis labels with detailed statistics. When table columns align with plot categories, readers can scan from plot to precise values without context switching.

```{r}
# Create aligned summary statistics
airquality_summary <- 
  airquality |>
  ...
  tidyr::pivot_longer(cols = -Month, names_to = "Measure", values_to = "Value") |>
...
# Create boxplot with matching x-axis
temp_plot <- ggplot(airquality, aes(x = month.name[Month], y = Temp)) +
  geom_boxplot(fill = "lightblue") +
  scale_x_discrete(limits = month.name[5:9]) +
  theme_minimal() +
  theme(axis.text.x = element_blank(), axis.title.x = element_blank()) +
  labs(y = "Temperature (°F)")

# Stack vertically with fixed table dimensions for alignment
temp_plot / wrap_table(airquality_summary, space = "fixed")
```

The table columns align directly beneath the corresponding boxplots, creating an integrated visualization where readers can see both the distribution shape and exact summary statistics.

### Adding titles and tags

Tables in patchwork compositions can receive titles, subtitles, and tags just like plots:

```{r}
pizza_plot + 
  wrap_table(pizza_table, space = "free_y") + 
  plot_annotation(
    title = "2015 Pizza Sales Analysis",
    subtitle = "Chicken and Supreme varieties",
    tag_levels = "A"
  )
```

Auto-tagging works naturally with tables, labeling them as figures in the sequence. This is particularly useful for academic publications where figures require alphabetic or numeric labels.

### Complex layouts

The **patchwork** package supports elaborate multi-panel layouts. Tables can occupy any position in the grid:

```{r}
# Second plot showing monthly totals
monthly_plot <-
  pizzaplace |>
  dplyr::mutate(month = lubridate::month(date, label = TRUE)) |>
  dplyr::filter(type %in% c("chicken", "supreme")) |>
  dplyr::count(month, type) |>
  ggplot(aes(x = month, y = n, fill = type)) +
  geom_col(position = "dodge") +
  labs(x = NULL, y = "Total Sold", fill = "Type") +
  theme_minimal()

# Arrange in a grid: plots on top, table spanning bottom
(pizza_plot | monthly_plot) / wrap_table(pizza_table, space = "free_y")
```

### Practical workflow

The typical workflow for creating combined figures involves three steps: build the table, build the plot(s), and compose them using **patchwork** operators. Keep each element as a separate object so you can adjust formatting independently before combining. Here's a template:
...
# 3. Compose
my_plot + wrap_table(my_table, space = "free_y")

# 4. Save as image
ggsave("combined_figure.png", width = 10, height = 6)
```

Note that saving uses `ggsave()` rather than `gtsave()` because the combined object is now a **patchwork** composition, not a pure **gt** table. The output is an image file containing both the plot and table as a single figure.
...
**Quality standards and presentation context close the decision.** For publication-quality output destined for print, LaTeX produces the most professional typography and precise positioning. High-resolution PNG images can match this quality and work in more contexts, though they lack LaTeX's mathematical typesetting sophistication. Web display always favors HTML and it's designed for screens, supports responsive layouts, and allows interactive exploration. For quick reviews, drafts, or iterative feedback, RTF and Word work fine despite their limitations since the goal is communication rather than perfection. Matching format quality to purpose prevents both over-engineering (LaTeX for an internal draft) and under-delivering (RTF for a final publication).
