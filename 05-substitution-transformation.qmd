...
You can customize the replacement text for specified columns:
...
Each column now displays its missing values differently: `num` shows `"N/A"`, `char` shows `"(none)"`, and `currency` shows an em dash. This contextual approach can make your tables more intuitive.

You can also target columns by type using tidyselect helpers like `where()`. This is useful when you want a consistent missing-value indicator for all columns of a particular class:
 
```{r}
exibble |>
  dplyr::select(num, char, currency) |>
  gt() |>
  sub_missing(columns = where(is.numeric), missing_text = md("*missing*")) |>
  sub_missing(columns = where(is.character), missing_text = "(empty)")
```

Here, all numeric columns receive *missing* text for missing values and all character columns show `"(empty)"` (though there's only a single character column in this example). Selecting by class means you don't need to enumerate column names, which is especially handy for wider datasets.
...
This applies different substitutions based on row context. Missing values in rows `"B"`, `"C"`, and `"D"` show `"pending"`, while the missing value in row `"E"` shows an em dash.
