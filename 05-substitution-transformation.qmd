...
Let's replace zero counts with an en-dash for clearer presentation:
...
dplyr::tibble(
  ...
  gt() |>
  fmt_number(columns = concentration, decimals = 3) |>
  sub_small_vals(columns = concentration, threshold = 0.01)
```

Values below `0.01` are replaced with "<0.01", acknowledging their presence without implying false precision. This is standard practice in scientific and analytical reporting.

The `small_pattern` argument customizes the replacement format:
...
dplyr::tibble(
  ...
  gt() |>
  fmt_number(columns = level_ppb, decimals = 2) |>
  sub_small_vals(
    columns = level_ppb, 
    threshold = 0.01,
    small_pattern = "< LOD"
...
Here, values below the threshold are replaced with "< LOD" (Limit of Detection), a common notation in analytical chemistry.
...
Values exceeding 100,000 are replaced with `"≥100,000"`, indicating they're beyond the displayed scale. This is useful for tables where exact values above a certain point aren't meaningful.

The pattern can be customized:
...
Values above the threshold show "> max", indicating they exceeded the measurement range.

### `sub_values()`

For more general substitution needs, `sub_values()` can replace any values matching specified conditions. This is the most flexible substitution function, accepting either specific values to match or a custom function.
...
Specific values are decorated with emoji indicators, making status categories visually distinct.
...
The function-based approach lets you define complex conditions. Here, any score below 50 is flagged with a warning.
...
Multiple values can be replaced with a single substitute, useful for grouping or categorization.
...
For targeted string replacements using regular expressions, `text_replace()` finds and replaces patterns within cell text.
...
The transformation applies to the stub column, shortening row labels from "row_1" to "R1".

### `text_case_when()`

For conditional text replacements based on complex logic, `text_case_when()` evaluates expressions and applies different replacements based on the results.
...
Different grades receive different decorations based on conditional logic. The `.default = NULL` ensures unmatched values pass through unchanged.

#### Numeric comparisons in text

Even though text transformations operate on strings, you can perform numeric comparisons:
...
This adds emoji indicators based on the underlying numeric values, even after currency formatting has been applied.
...
Each status value is transformed into a more descriptive, emoji-decorated representation.

### `text_case_match()`

For simpler one-to-one replacements without complex conditions, `text_case_match()` provides a cleaner syntax similar to `dplyr::case_match()`.
