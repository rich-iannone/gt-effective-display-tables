...
locales <- c("en-US", "de-DE", "fr-FR", "pt-BR", "hi-IN", "ja-JP", "it-IT", "de-CH")
languages <- c("English", "German", "French", "Portuguese", "Hindi", "Japanese", "Italian", "German")
...
countrypops |>
  ...
  dplyr::mutate(
    locale = locales[match(country_name, countries)],
    language = languages[match(country_name, countries)]
  ) |>
  dplyr::rowwise() |>
  dplyr::mutate(
    formatted_population = vec_fmt_integer(population, locale = locale)
  ) |>

  dplyr::ungroup() |>
  dplyr::select(country_name, language, locale, formatted_population) |>
  gt() |>
  tab_header(
  ...
  cols_label(
    ...
    language = "Language",
    locale = "Locale",
    formatted_population = "Population (2024)"
  ) |>
  cols_align(align = "right", columns = formatted_population) |>
